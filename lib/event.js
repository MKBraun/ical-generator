        ...
        if(data.repeating) {
            ...
            if(data.repeating.exclude) {
                data.repeating.exclude.forEach(function(excludedDate) {
                    //START workaround
                    console.log("TEST:");
                    console.log(excludedDate.getHours());
                    excludedDate.setUTCHours(data.start.getUTCHours());
                    excludedDate.setUTCMinutes(data.start.getUTCMinutes());
                    excludedDate.setUTCSeconds(data.start.getUTCSeconds());
                    console.log(excludedDate.getHours());
                    //END workaround
                    g += tools.formatDateTZ('EXDATE', excludedDate, data);    // date without time in EXDATE property
                    g += '\r\n';
                });
